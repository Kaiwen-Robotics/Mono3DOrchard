...
class ShapeCompletionDataset:
    ...
    def __init__(
        ...
        assert (
            return_pcd or return_rgbd
        ), "return_pcd and return_rgbd are set to False. Set at least one to True"
    ...
    def get_gt(self, fid):
        return o3d.io.read_point_cloud(
            os.path.join(self.fruit_list[fid]["path"], "gt/pcd/fruit.ply")
        )
    ...
    def get_rgbd(self, fid):
        ...
        frames = os.listdir(os.path.join(fid_root, "input/mask/"))

        for frameid in frames:

            # pose_path = os.path.join(
            #     fid_root, "input/poses/", frameid.replace("png", "txt")
            # )
            pose_path = os.path.join(
                fid_root, "input/pose/", frameid.replace("png", "txt")
            )
            ...
            depth_path = os.path.join(
                fid_root, "input/depth/", frameid.replace("png", "npy")
            )
            ...
            mask_path = os.path.join(fid_root, "input/mask/", frameid)
            # print(mask_path)
