...
@click.option(
    ...
    default=join(
        dirname(abspath(__file__)), "configs/shape_completion_challenge_pepper.yaml"
    ),
)
def main(config):
    ...
    mesh_extractor = MeshExtractor(
        ...
        cube_radius=object_radius_max_m,
    )  # mc res: 0.2/40 ~ 5mm

    if cfg["vis"]["vis_on"]:
        vis = OptVisualizer(
            object_radius_max_m * 1.2, pause_time_s=cfg["vis"]["vis_pause_s"]
        )
    else:
        vis = None
    ...
    fruit_comp_data = ShapeCompletionDataset(
        cfg["data_dir"], cfg["split"]
    )  # train, val, test
    depth_scale = 1.0
    cur_submap_id = 1  # only one target

    result_folder = os.path.join(
        cfg["data_dir"], "results", cfg["run_name"], cfg["split"]
    )
    access = 0o755
    ...
    for cur_fruit_data in fruit_comp_data:
        ...
        sample_frame_idx = np.linspace(
            0, frame_count - 1, min(frame_count, cfg["frame_per_fruit"])
        ).astype(np.int32)
        ...
        # print("Point count point:", map_pcd)
        ...
        print(down_point_count, original_point_count)
        map_pcd = map_pcd.random_down_sample(
            sampling_ratio=min(down_point_count / original_point_count, 1.0)
        )
        ...
        for idx in tqdm(sample_frame_ids):  # for each frame
            ...
            depth_imgs[img_id_str] = (
                cur_fruit_rgbd_frame["depth"] / depth_scale
            )  # unit: m
            rgb_imgs[img_id_str] = cur_fruit_rgbd_frame["rgb"]  # 0-255
            cam_poses[img_id_str] = cur_fruit_rgbd_frame[
                "pose"
            ]  # current camera extrinsic
        ...
        if cfg["vis"]["vis_on"]:
            ...
            print(render_data["frame_id"], mid_idx)
            if render_data["frame_id"] == []:
                continue
            ...
            rgb_img[
                ~cur_fruit_mask
            ] *= 0.4  # for visualization only (highlight masked part)
            rgb_img[
                depth_img == 0
            ] *= 0.7  # for visualization only (highlight the part with valid depth)
            # visualize the fg and bg samples
            if cfg["vis"]["show_pix_sample"]:
                rgb_img[cur_pix_fg[:, 1], cur_pix_fg[:, 0]] = np.array(
                    [0, 0, 255]
                )  # fg samples
                rgb_img[cur_pix_bg[:, 1], cur_pix_bg[:, 0]] = np.array(
                    [255, 0, 0]
                )  # bg samples
        ...
        mean_color = np.mean(
            np.array(map_pcd.colors), axis=0
        )  # use avaerge color of the point cloud
        cur_color = color_table[0]  # use random color

        cur_pcd_w = copy.deepcopy(map_pcd)
        points_w_torch = torch.tensor(
            np.array(cur_pcd_w.points), device=dev, dtype=dtype
        )

        T_wo_torch = torch.eye(4, device=dev, dtype=dtype)  # initial guess scale TODO
        ...
        if deepsdf_baseline:
            latent, _, iter_count = opt.shape_opt_deepsdf(
                latent, T_ow_torch, points_w_torch, mean_color
            )
        else:  # ours
            latent, T_ow_torch, iter_count = opt.shape_pose_joint_opt(
                latent,
                T_ow_torch,
                render_data,
                points_w_torch,
    ...
    if cfg["vis"]["wandb_log_on"]:
        setup_wandb()
        wandb.init(
            project="HOMA", config=cfg, dir=cfg["data_dir"]
        )  # your own worksapce
